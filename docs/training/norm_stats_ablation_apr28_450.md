...
> **创建日期**：2026-04-30
> **状态**：✅ 两组均已完成 (2026-04-30 19:56 CST 最后一组 new_norm 落地)。
...
| 完成时间 | **Thu 19:56 CST** ✅ | **Thu 11:29 CST** ✅ |
| 总耗时 | **32:53 hr** (gf1 步速 ~3.0 s/step) | **23:55 hr** (gf0 步速 2.0 s/step) |
...
### A 组 new_norm (gf1) — ✅ 已完成 (2026-04-30 19:56 CST, 32:53 hr)

| step | MAE@1 | @10 | @25 | @50 | Δ vs prev |
|---:|---:|---:|---:|---:|---:|
| 2000 | 0.0182 | 0.0437 | 0.0827 | 0.1292 | (起点) |
| 4000 | 0.0174 | 0.0402 | 0.0751 | 0.1177 | -4.4% |
| 6000 | 0.0167 | 0.0370 | 0.0677 | 0.1055 | -4.0% |
| 8000 | 0.0161 | 0.0344 | 0.0617 | 0.0957 | -3.6% |
| 10000 | 0.0154 | 0.0323 | 0.0576 | 0.0889 | -4.3% |
| 12000 | 0.0148 | 0.0310 | 0.0551 | 0.0850 | -3.9% |
| 14000 | 0.0143 | 0.0301 | 0.0536 | 0.0826 | -3.4% |
| 16000 | 0.0138 | 0.0295 | 0.0527 | 0.0813 | -3.5% |
| 18000 | 0.0135 | 0.0293 | 0.0523 | 0.0806 | -2.2% |
| 20000 | 0.0132 | 0.0290 | 0.0520 | 0.0801 | -2.2% |
| 22000 | 0.0131 | 0.0290 | 0.0519 | 0.0799 | -0.8% |
| 24000 | 0.0129 | 0.0289 | 0.0518 | 0.0797 | -1.5% |
| 26000 | 0.0128 | 0.0289 | 0.0517 | 0.0796 | -0.8% |
| **28000** | **0.0127** | **0.0289** | **0.0518** | **0.0796** | -0.8% **★ best** |
| 29999 | 0.0127 | 0.0290 | 0.0518 | 0.0796 | 0.0% (final, tied) |

**最终结果**: best **MAE@1 = 0.0127** at step 28000 (tied with step 29999, 但 28000 在 @25 上 0.0518 vs 0.0518 几乎一致, @50 一致). 推荐部署 **step 28000** (mid-plateau, watcher 自动选)。step 26000-29999 全 plateau @ 0.0127-0.0128。
...
| 26000 | 0.0128 | 0.0141 | -9% |
| **28000** | **0.0127** | **0.0140** | **-9.3%** (final-plateau) |
| 29999 | 0.0127 | 0.0140 | -9.3% (final) |

### 关键观察

1. **new_norm 在每个 step 都更低 MAE**: 从 step 2000 起就一致优于 inherit_norm, **差距稳定在 9-16%**
2. **gap 随训练缓慢缩窄**: 早期 16% (step 2k) → 后期稳定 9-9.3% (step 22k+); **不会完全收敛到 0** — 模型无法完全"补偿"错误的 norm
3. **两组都已 plateau**:
   - inherit_norm: step 26000-29999 稳在 0.0140-0.0141
   - new_norm: step 26000-29999 稳在 0.0127-0.0128
4. **plateau gap 收敛到 -9.3%** (final 0.0127 vs 0.0140) — 这是长训下 norm_stats 选择对 MAE@1 的**永久代价**

### 最终对照 (final step 29999, 两组均已完成)

| 指标 | A new_norm (best step 28000) | B inherit_norm (best step 28000) | gap |
|---|---:|---:|---:|
| MAE@1 | **0.0127** | **0.0140** | **-9.3%** ⭐ |
...
**绝对值意义**: new_norm final 0.0127 在 mix_apr28_450 val 上**接近 mixed_gf0_173 (0.0129, 不同 val) 的水平**, 是 Task A 系列中 single-step accuracy 最好的训练之一。
...
**部署 ready 文件夹** (按 mixed_1 init 模型的部署格式整理, 2026-04-30 21:36 CST):
- 路径: `/vePFS/tim/workspace/deepdive_kai0_tmp/data/mix_apr28_450_best_step28000/`
- 结构: `_CHECKPOINT_METADATA + norm_stats.json (顶层) + params/ + README.md`
- 大小: 12 GB

**部署 tar 包** (重新打包, 含 README.md):
- 路径: `/vePFS/tim/workspace/deepdive_kai0_tmp/data/mix_apr28_450_best_step28000.tar`
- 大小: 11.6 GB (12,440,412,160 bytes)
- sha256: `25ca197e1aba2bf830789b57a429c24af0b1eec9388fbfb78a5a677b385533ed`
- MAE@1 = 0.0127, @10 = 0.0289, @25 = 0.0518, @50 = 0.0796

**Note**: auto-pack watcher 早期写过的 `mix_apr28_450_new_norm_best.tar` 是 step 28000 但缺顶层 `norm_stats.json` 和 `README.md`。**正式部署用 `mix_apr28_450_best_step28000.tar`**。
...
| **2026-04-30 19:56 CST** | **gf1 new_norm 完成** (step 29999 final, best step 28000 @ MAE 0.0127, 总耗时 32:53 hr) |
| 2026-04-30 19:56 CST | new_norm best ckpt 自动打包 (auto-pack watcher) → `mix_apr28_450_new_norm_best.tar` |
| 2026-04-30 21:36 CST | 重新整理 deployment-ready 文件夹 (按 mixed_1 init 模型格式: 顶层 norm_stats.json + README.md) → `mix_apr28_450_best_step28000.tar` |
